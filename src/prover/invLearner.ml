...
(* This is not used anymore. Just here for nostalgic value.
...
let get_more_models session model gts =
  ...
  let get_model_for_equality (model_num, session) eq =
	...
	(* Also output the model in html *)
	let model_chan = open_out (file_name ^ ".html") in
	Model.output_html model_chan model gts;
	close_out model_chan;
