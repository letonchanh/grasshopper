...
let rec string_of_ext_value = function
  | BaseVal v -> Printf.sprintf "BaseVal (%s)" (string_of_value v)
  | MapVal (vmap, ev) ->
     let string_of_value_list vals = String.concat ", " (List.map string_of_value vals) in
     Printf.sprintf
       "MapVal ([%s], %s)"
       (String.concat ", " (ValueListMap.fold (fun keys value res -> (Printf.sprintf "[%s] |-> %s" (string_of_value_list keys) (string_of_value value))::res) vmap []))
       (string_of_ext_value ev)
  | SetVal vals ->
     Printf.sprintf
       "SetVal {%s}" (String.concat ", " (ValueSet.fold (fun value res -> (string_of_value value)::res) vals []))
  | TermVal (args, term) ->
     Printf.sprintf
       "TermVal ([%s], %s)"
       (String.concat ", " (List.map (fun (id, srt) -> Printf.sprintf "%s (%s)" (string_of_ident id) (string_of_sort srt)) args))
       (string_of_term term)
  | FormVal (args, form) ->
     Printf.sprintf
       "FormVal ([%s], %s)"
...
let string_of_interpretation intp =
  let res =
    SortedSymbolMap.fold
      (fun (id, (argsrts, retsrt)) (vmap, value) res ->
       let string_of_value_list vals = String.concat ", " (List.map string_of_value vals) in
       let str =
	 Printf.sprintf
	   "%s(%s) : %s  |->  ([%s], %s)"
	   (string_of_symbol id)
	   (String.concat ", " (List.map string_of_sort argsrts))
	   (string_of_sort retsrt)
	   (String.concat ", " (ValueListMap.fold (fun keys value res -> (Printf.sprintf "[%s] |-> %s" (string_of_value_list keys) (string_of_value value))::res) vmap []))
	   (string_of_ext_value value) in
       str::res)
      intp
      [""] in
  String.concat "\n" res
...
let string_of_sorted_value srt v =
  let rec elim_loc = function
    ...
    | Map (dsrt, rsrt) -> Map (List.map elim_loc dsrt, elim_loc rsrt)
    | srt -> srt
  in
  match srt with
...
let output_json chan model =
  ...
  let output_value sym srt =
    ...
    | Map (dsrt, rsrt) ->
        ...
        let args = Util.flat_map (get_values_of_sort model) dsrt in
        let arg_res = 
          Util.flat_map 
            (fun arg ->
              try
...
let output_txt chan model =
  ...
  let output_loc loc l_id srt chan =
    let output_data_fields loc rsrt =
      SymbolSet.iter
       (fun fld ->
        try
           ...
           let fld_str = string_of_symbol fld in
           let m, d = find_map_value model f (Loc srt) rsrt in
	   (*let _ = Format.fprintf Format.std_formatter "trying1\n" in *)
           let v = fun_app model (MapVal (m, d)) [loc] in
	   (*let _ = Format.fprintf Format.std_formatter "trying2\n" in *)
	   Printf.fprintf chan "(%d, %s, I %s)\n" l_id fld_str (string_of_value v)
			  (*Format.fprintf Format.std_formatter "      <tr><td><b>%d : %s = %s</b></td></tr>\n" l_id fld_str (string_of_value v) *)
  ...
  let process_srt srt id_map =
    ...
    let output_flds rsrt id_map =
      ...
      let max_fld_id_map =
        List.fold_left
          (fun max_fld_id_map f ->
           let fld = find_term model f fld_srt in
           match fld with
           | Var ((fName, fId), _) ->
              begin
                let curMaxId =
      ...
      List.fold_left
        (fun id_map f ->
	      ...
	      let id_map, r_id = get_id_of_loc id_map rsrt r in
              let fld = find_term model f fld_srt in
	      let f_str = string_of_term fld in
              let doPrint =
                match fld with
                | Var ((fName, fId), _) -> fId = StringMap.find fName max_fld_id_map
                | _ -> true in
	      (* Don't print null.next *)
	      if doPrint && l_id != 0 then
		begin
		  output_loc l l_id srt chan;
		  Printf.fprintf chan "(%d, %s, A %d)\n" l_id f_str r_id
		end;
	      id_map
	...
	(fun sym id_map ->
	 match sym with
	 | FreeSym (name, num) ->
	    if num == StringMap.find name idname_to_latest_ssa_id then
	      begin
		let l = interp_symbol model sym ([], Loc srt) [] in
		let id_map, l_id = get_id_of_loc id_map srt l in

		Printf.fprintf chan "[%s : A %d]\n" name l_id;
		id_map
	      end
	    else
	      id_map
